// ============================================
// COMPLETE FIX FOR STREAK TIMEZONE ISSUE
// ============================================
...
if (path === "/analytics/streaks" && method === "GET") {
  ...
  const timezone = url.searchParams.get("timezone") || "UTC"; // ✅ Extract timezone
  if (!uid) return this.json({ error: "uid required" }, 400);
  return this.getStreaks(uid, timezone); // ✅ Pass timezone to method
}

// 2. UPDATE YOUR getStreaks METHOD
// ============================================
async getStreaks(uid, timezone = 'UTC') { // ✅ Accept timezone parameter
  try {
...
// WHAT CHANGED:
// ============================================
// 1. Route handler now extracts and passes timezone parameter
// 2. getStreaks() accepts timezone parameter (defaults to 'UTC')
// 3. Uses user's timezone to calculate "today"
// 4. Calculates brokenStreak before checking isStreakActive
// 5. Sets currentStreak = brokenStreak when streak is broken
//
// RESULT:
// - currentStreak shows 10 (your broken streak) instead of 0
// - Timezone is properly handled for Asia/Calcutta users
// - Streak recovery modal shows correct values
