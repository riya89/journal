  ...
  if (authHeader && authHeader.startsWith('Bearer ')) {
    ...
    if (token) {
      try {
        ...
        if (!env.LM_AUTH_ALLOWED_ORIGINS) {
          throw new Error('LM_AUTH_ALLOWED_ORIGINS environment variable not configured');
        }
        if (!env.LM_AUTH_ALLOWED_ISSUERS) {
          throw new Error('LM_AUTH_ALLOWED_ISSUERS environment variable not configured');
        }
        ...
        if (env.logger) {
          try {
            ...
            env.logger.error('JWT verification failed with error during logging', { error: String(error) });
          }
        }
      }
    }
