...
export function generateInsights(stats, comparison = null, moodData = [], streakInfo = null) {
  ...
  const actualCurrentStreak = streakInfo?.currentStreak || 0;
  const streakToShow = actualCurrentStreak > 0 ? actualCurrentStreak : patterns.highStreakLength;
  const hasStreak = streakToShow >= 3;

  if (hasStreak) {
    const isCurrentStreak = actualCurrentStreak > 0 || patterns.currentStreak > 0;
    const message = isCurrentStreak
      ? `You have a ${streakToShow}-day streak of good moods! Keep it going! 🔥`
      : `You had a ${streakToShow}-day streak of good moods! What were you doing during that time?`;
